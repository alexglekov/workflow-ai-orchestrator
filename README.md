...
MVP: пользователь описывает задачу текстом, система собирает последовательность шагов и выполняет их через отдельные коннекторы (Mail, Telegram, 1С, Excel, Web).
...
5. Добавьте триггер (расписание, опрос почты или webhook) или запустите вручную с JSON input.
6. На экране запуска видны статусы, результат или ошибка каждого шага. Упавший run можно повторить.

Запуск: вручную, по расписанию, по опросу почты или через `POST /api/hooks/:token`.

В шагах доступны плейсхолдеры `{{previous}}`, `{{item.field}}`, `{{input.field}}`, `{{steps.1.field}}`.
...
### Web (поиск и страница)

Для публичных справок без отдельного коннектора: ИНН, BestChange, открытые сайты. CRM нет — лид и задача пишутся в 1С.

- `web.search` — поиск (DuckDuckGo; опционально Brave Search API)
- `web.fetch` — скачать URL и вернуть текст и таблицы

Подключение необязательно. Приватные адреса и localhost закрыты. Instagram/личные кабинеты этим шагом не открыть.

Пример: поиск → fetch → Telegram или `onec.create_record`.
